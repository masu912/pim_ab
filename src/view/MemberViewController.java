...
public class MemberViewController implements Initializable {
	...
	@FXML	private TextField tfContect;
	...
	@FXML	private TableColumn<Member, String> columnContect;
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		columnContect.setCellValueFactory(cvf -> cvf.getValue().contectProperty());
	...
	private void showMemberInfo(Member member) {
		if (member != null) {
			...
			tfContect.setText(member.getContect());
		}
		 else {
			 tfID.setText("");
			 tfPW.setText("");
...
//		     tfMobilePhone.setText("010");
	...
	private void handleCreate() { // event source, listener, handler
		if(tfID.getText().length() > 0) {
			Member newMember = 
					new Member(tfID.getText(), tfPW.getText(), tfName.getText(),tfContect.getText());;
			data.add(newMember);			
			tableViewMember.setItems(data);
